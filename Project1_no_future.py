...
def build_name_frame(names: list[str]) -> pd.DataFrame:
...
def summarise_name_frame(frame: pd.DataFrame, *, top_n: int = 10) -> dict[str, Any]:
...
def _generate_wordcloud(names: list[str], output_dir: Path) -> Path:
...
def create_visualisations(
    frame: pd.DataFrame, names: list[str], output_dir: Path
) -> list[Path]:
